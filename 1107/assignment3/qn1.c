...

{
  ...
  {
    return 0; // not identical as both must be null, so return
  }
  ...
  while (tree1 != NULL && tree2 != NULL)
  {
    ...
    else // if identical check child nodes
    {
      int k = identical(tree1->left, tree2->left); // the function will check the child loops of the child loop THUS recursion 
      if ( k != 0)
      {
        int k = identical(tree1->right, tree2->right);
      }
      return k;
    }
  }
