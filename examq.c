...
void reverse(Queue *q)
{
  Stack s;
  s.ll.head = NULL;
  if ((q->ll).head == NULL ) return; // if null quit
  int size = q->ll.size;
  int size2 = q->ll.size;
  int count = 0;
 // stack shld be 6 3 5 4 2 1
  while (size != 0)
  {
    int temp = dequeue(q);
    push(&s, temp);
  ...
  {
    enqueue(q, pop(&s)); // thru this loop q becomes 6 5 4 3 2 1
  }
  while(count != size2)
  {
    push(&s, dequeue(q)); // 6 is in stack 
    int half = q->ll.size/2;   
    while (half != 0)
    {
      int temp = dequeue(q);
      enqueue(q, temp);
      half--; // q becomes 3 2 1 5 4 6 2nd loop, 5 4 6 3 2 1
    }
    count++;
  }
  while (s.ll.head != NULL)
